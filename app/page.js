...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setMerchant(parsed.merchant || merchant)
      } catch {}
    }
    ...
    if (rememberedEmail) {
      setAuth(current => ({ ...current, email: rememberedEmail }))
    }
    ...
    supabase.rpc('get_public_order', { p_token: token }).maybeSingle()
      .then(({ data, error }) => {
        ...
        setDraft(current => ({
          ...
          delivery: String(data.delivery_fee),
          address: data.delivery_address || '',
        }))
  ...
  useEffect(() => {
    ...
    fetch(`/api/paystack/verify?reference=${encodeURIComponent(reference)}`)
      ...
      .catch(() => {
        setScreen('payment')
        setNotice('Payment verification failed. Please try again.')
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    const loadAccount = async () => {
      ...
      if (profile) {
        setMerchant(current => ({
          ...current,
          name: profile.full_name || 'Merchant',
          business: profile.business_name || 'My Store',
          phone: profile.phone || '',
  ...
  useEffect(() => {
    localStorage.setItem(
      'orderflow-state',
      JSON.stringify({ orders, merchant })
    )
  }, [orders, merchant])

  const total = useMemo(
    () => (
      Number(draft.price || 0) * Number(draft.qty || 1)
      + Number(draft.delivery || 0)
    ),
    [draft]
  )
  ...
  const signIn = async () => {
    ...
    if (rememberMe) {
      localStorage.setItem(
        'orderflow-remembered-email',
        auth.email
      )
    } else {
      localStorage.removeItem('orderflow-remembered-email')
    }
  ...
  const signInWithGoogle = async () => {
    if (!supabase) {
      return setNotice(
        'Connect Supabase to activate Google sign-in.'
      )
    }
  ...
  const resetPassword = async () => {
    ...
    if (!supabase) {
      return setNotice(
        'Password reset becomes available when Supabase is connected.'
      )
    }
  ...
  const confirmBuyerOrder = async () => {
    if (supabase && selected?.publicToken) {
      const { data, error } = await supabase.rpc(
        ...
        {
          p_token: selected.publicToken,
        }
      )

      if (error) {
        return setNotice(
          'We could not confirm this order. Please try again.'
        )
      }

      if (!data && selected.status !== 'Confirmed') {
        return setNotice(
  ...
  const updateOrderStatus = async () => {
    ...
    setOrders(orders.map(order => (
      order.id === selected.id ? updated : order
    )))
  ...
  const refreshTracking = async () => {
    if (!supabase || !selected?.publicToken) {
      return setNotice(
        'This order does not have a secure tracking link.'
      )
    }
    ...
    if (error || !data) {
      return setNotice(
        'The latest status could not be loaded. Please try again.'
      )
    }
    ...
    setMerchant(current => ({
      ...current,
      phone: data.merchant_phone || current.phone,
    }))
  ...
  const contactSeller = () => {
    ...
    const message = encodeURIComponent(
      `Hello, I am contacting you about OrderFlow order ${selected?.id || ''}.`
    )
    ...
    setEditingOrder({
      ...
      unitPrice: Number(
        ...
        ?? Math.max(
          ...
          - Number(selected.deliveryFee || 3500),
          0
        ) / Number(selected.qty || 1)
      ),
  ...
  const saveOrderEdits = async () => {
    const quantity = Math.max(
      1,
      Number(editingOrder.qty || 1)
    )

    const unitPrice = Math.max(
      0,
      Number(editingOrder.unitPrice || 0)
    )

    const deliveryFee = Math.max(
      0,
      Number(editingOrder.deliveryFee || 0)
    )

    if (
      !editingOrder.name?.trim()
      || !editingOrder.item?.trim()
    ) {
      return setNotice(
        'Customer name and item are required.'
      )
    }
    ...
    if (
      ...
      const { error } = await supabase
        .from('orders')
        .update({
          customer_name: editingOrder.name.trim(),
          customer_phone: editingOrder.phone || '',
          item_name: editingOrder.item.trim(),
          quantity,
          unit_price: unitPrice,
          delivery_fee: deliveryFee,
          updated_at: new Date().toISOString(),
        })
        .eq('id', editingOrder.databaseId)
    ...
    const updated = {
      ...
      unitPrice,
      deliveryFee,
      amount: unitPrice * quantity + deliveryFee,
    }
  ...
  const cancelOrder = async () => {
    if (
      !window.confirm(
        `Cancel order ${selected.id}? This action will be shown to the buyer.`
      )
    ) {
      return
    }
  ...
  const startPayment = async () => {
    if (paymentMethod === 'opay') {
      return setNotice(
        'OPay setup requires approved merchant API credentials. Choose Paystack or Bank transfer for this test payment.'
      )
    }

    if (paymentMethod === 'escrow') {
      return setNotice(
        'Escrow is shown as a planned protected-payment option. It will be activated after the holding and release process is completed.'
      )
    }
    ...
    try {
      ...
      if (!contentType.includes('application/json')) {
        throw new Error(
          response.status === 404
            ? 'Payment service is not available in this deployment. Redeploy the API route and try again.'
  ...
  if (screen === 'splash') {
    return (
      <main className="splash">
        <OrderFlowLogo />
        <p>Orders made simple.</p>
        <div className="splash-pulse" />
      </main>
    )
  }

  if (screen === 'public-loading') {
    return (
      <main className="splash">
        <OrderFlowLogo />
        <p>
          {publicOrderLoading
            ? 'Opening your secure order…'
            : 'Loading order…'}
        </p>
        <div className="splash-pulse" />
      </main>
  ...
  if (screen === 'public-error') {
    return (
      <AppShell onBack={go}>
        <section className="success">
          <div className="order-link-error">!</div>
      ...
      <AppShell onBack={go}>
        <section className="welcome">
          ...
          <div
            ...
            aria-hidden="true"
          >
            <span className="flow-card flow-card-one">
              New order
            </span>
            <span className="flow-card flow-card-two">
              Payment secured
            </span>
            <span className="flow-card flow-card-three">
              Ready to deliver
            </span>
          ...
          <h1>
            Turn every DM into a confirmed order.
          </h1>

          <p>
            Create orders, receive secure payments
            and keep every customer updated in one
            place.
          </p>
          ...
          <small className="welcome-trust">
            Built for independent sellers and growing
            businesses.
          </small>
        </section>
      </AppShell>
      ...
      >
        <section className="form auth-form">
          ...
          <div className="auth-intro">
            <h1>Welcome back</h1>
            <p>
              Sign in to manage orders, buyers and
              payments.
            </p>
          </div>
          ...
          <button
            className="social"
            disabled
            title="Apple Developer membership is required"
          >
          ...
          {!isSupabaseConfigured && (
            <small className="center">
              Demo mode is active until Supabase is
              connected.
            </small>
          )}
        </section>
  ...
  if (screen === 'dashboard') return <AppShell onBack={go} merchantHeader merchantName={merchant.name} onNotifications={() => setNotificationsOpen(value => !value)} onProfile={() => go('profile')}><section className="dashboard"><div className="dash-head"><div><p>Good afternoon, {merchant.name}</p><h1>Orders</h1></div></div>{notificationsOpen && <aside className="notification-panel"><div><strong>Notifications</strong><button onClick={() => setNotificationsOpen(false)} aria-label="Close notifications">×</button></div><p><b>Payment received</b><span>An order payment has been confirmed.</span></p><p><b>Order update</b><span>Review your awaiting orders.</span></p></aside>}<CommerceBanner /><button onClick={() => go('create')}>+ Create new order</button><div className="stats"><div><span>Awaiting</span><b>{orderStats.awaiting}</b></div><div><span>Active</span><b>{orderStats.active}</b></div><div><span>Issues</span><b>{orderStats.issues}</b></div></div><div className="section-title"><h2>Recent orders</h2><button className="section-link" onClick={() => go('orders')}>View all</button></div><div className="orders">{orders.slice(0,3).map(order => <button className="order" key={order.id} onClick={() => { setSelected(order); go('merchant-order') }}><div><b>{order.id} · {order.name}</b><span>{order.qty} item{order.qty > 1 ? 's' : ''} · {naira.format(order.amount)}</span><small>{order.channel} · {orderDate.format(new Date(order.createdAt || Date.now()))}</small></div><em className={order.status.toLowerCase().replaceAll(' ','-')}>{order.status}</em></button>)}{orders.length === 0 && <div className="empty-state"><strong>No orders yet</strong><span>Create your first order to see it here.</span></div>}</div></section><MerchantNav active="dashboard" onNavigate={go}/></AppShell>
