...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadAccount = async () => {
      ...
      if (session.user.user_metadata?.account_type === 'buyer') { window.location.assign('/buyer'); return }
  ...
  const saveOrder = async () => {
    if (supabase && session) {
      ...
      const { data, error } = await supabase.from('orders').insert({
        ...
        line_items: newItems.map(i=>({name:i.name.trim(),quantity:Number(i.quantity),unit_price:Number(i.unit_price)})),
