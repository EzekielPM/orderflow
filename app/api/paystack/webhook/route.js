...
export async function POST(request) {
  ...
  if (!secretKey) return NextResponse.json({ message: 'Webhook is not configured.' }, { status: 503 })
  ...
  if (!validSignature) return NextResponse.json({ message: 'Invalid signature.' }, { status: 401 })
